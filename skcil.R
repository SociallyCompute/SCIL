...
el <- read.csv("data/FileRevisions.csv")

#el$source <- as.character(el$title)
#el$target <- as.character(el$last_modifying_user_name)

el$source <- el$title
el$target <- el$last_modifying_user_name
